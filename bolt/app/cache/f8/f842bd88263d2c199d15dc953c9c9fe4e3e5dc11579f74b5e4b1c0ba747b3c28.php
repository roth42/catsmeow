<?php

/* @BoltProfiler/toolbar/bolt.html.twig */
class __TwigTemplate_d80804982ed8c8156f16ad0f2f2f8e8847aae4e8195c8e6a3651bc6093a42498 extends Twig_Template
{
    public function __construct(Twig_Environment $env)
    {
        parent::__construct($env);

        $this->blocks = array(
            'toolbar' => array($this, 'block_toolbar'),
        );
    }

    protected function doGetParent(array $context)
    {
        // line 1
        return $this->loadTemplate((((($this->getAttribute($this->getAttribute($this->getContext($context, "app", true), "request", array(), "any", false, true), "isXmlHttpRequest", array(), "any", true, true)) ? (_twig_default_filter($this->getAttribute($this->getAttribute($this->getContext($context, "app", true), "request", array(), "any", false, true), "isXmlHttpRequest", array()), false)) : (false))) ? ("@WebProfiler/Profiler/ajax_layout.html.twig") : ("@WebProfiler/Profiler/layout.html.twig")), "@BoltProfiler/toolbar/bolt.html.twig", 1);
    }

    protected function doDisplay(array $context, array $blocks = array())
    {
        $this->getParent($context)->display($context, array_merge($this->blocks, $blocks));
    }

    // line 3
    public function block_toolbar($context, array $blocks = array())
    {
        // line 4
        echo "<style>
.sf-minitoolbar {
    background-color: transparent;
    background-image: url(data:image/png;base64,) ;
    background-size: 32px 32px;
    background-repeat: no-repeat;
}
.sf-minitoolbar img {
    border: 1px dashed #0F0;
    opacity: 0;
}
</style>

    ";
        // line 17
        ob_start();
        // line 18
        echo "        <img width=\"32\" height=\"32\" alt=\"Bolt\" style=\"width: 32px; height: 32px; margin-top: -2px;\"
            src=\"data:image/png;base64,\">

        <span></span><strong>Bolt ";
        // line 21
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        echo twig_escape_filter($this->env, $this->getAttribute($_collector_, "version", array()), "html", null, true);
        echo "</strong></span>

        ";
        // line 23
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        if ( !twig_test_empty($this->getAttribute($_collector_, "versionname", array()))) {
            // line 24
            echo "            <span class=\"sf-toolbar-status sf-toolbar-status-yellow\">";
            if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
            echo twig_escape_filter($this->env, $this->getAttribute($_collector_, "versionname", array()), "html", null, true);
            echo "</span>
        ";
        }
        // line 26
        echo "
    ";
        $context["icon"] = ('' === $tmp = ob_get_clean()) ? '' : new Twig_Markup($tmp, $this->env->getCharset());
        // line 28
        echo "
    ";
        // line 29
        ob_start();
        // line 30
        echo "
        <div class=\"sf-toolbar-info-piece\">

            <strong>Bolt - </strong> ";
        // line 33
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        echo twig_escape_filter($this->env, $this->getAttribute($_collector_, "payoff", array()), "html", null, true);
        echo "<br>

            ";
        // line 35
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        echo twig_escape_filter($this->env, $this->getAttribute($_collector_, "fullversion", array()), "html", null, true);
        echo " - ";
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        echo $this->getAttribute($_collector_, "aboutlink", array());
        echo " - Visit <a href=\"http://bolt.cm\" target=\"_blank\">Bolt.cm</a></strong> <br>

            ";
        // line 37
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        if ( !twig_test_empty($this->getAttribute($_collector_, "branding", array()))) {
            // line 38
            echo "                ";
            if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
            echo $this->getAttribute($_collector_, "branding", array());
            echo "
            ";
        }
        // line 40
        echo "
        </div>

        ";
        // line 43
        if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
        if ($this->getAttribute($_collector_, "editlink", array())) {
            // line 44
            echo "        <div class=\"sf-toolbar-info-piece\">
            <hr style=\"margin: 0;\">
            Edit: <a href=\"";
            // line 46
            if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
            echo twig_escape_filter($this->env, $this->getAttribute($_collector_, "editlink", array()), "html", null, true);
            echo "\">";
            if (isset($context["collector"])) { $_collector_ = $context["collector"]; } else { $_collector_ = null; }
            echo twig_escape_filter($this->env, _twig_default_filter($this->env->getExtension('Bolt')->excerpt($this->getAttribute($_collector_, "edittitle", array()), 40), "(no title)"), "html", null, true);
            echo "</a>
        </div>
        ";
        }
        // line 49
        echo "

    ";
        $context["text"] = ('' === $tmp = ob_get_clean()) ? '' : new Twig_Markup($tmp, $this->env->getCharset());
        // line 52
        echo "    ";
        if (isset($context["profiler_url"])) { $_profiler_url_ = $context["profiler_url"]; } else { $_profiler_url_ = null; }
        $this->loadTemplate("@WebProfiler/Profiler/toolbar_item.html.twig", "@BoltProfiler/toolbar/bolt.html.twig", 52)->display(array_merge($context, array("link" => $_profiler_url_)));
    }

    public function getTemplateName()
    {
        return "@BoltProfiler/toolbar/bolt.html.twig";
    }

    public function isTraitable()
    {
        return false;
    }

    public function getDebugInfo()
    {
        return array (  134 => 52,  129 => 49,  119 => 46,  115 => 44,  112 => 43,  107 => 40,  100 => 38,  97 => 37,  88 => 35,  82 => 33,  77 => 30,  75 => 29,  72 => 28,  68 => 26,  61 => 24,  58 => 23,  52 => 21,  47 => 18,  45 => 17,  30 => 4,  27 => 3,  18 => 1,);
    }
}
/* {% extends app.request.isXmlHttpRequest|default(false) ? '@WebProfiler/Profiler/ajax_layout.html.twig' : '@WebProfiler/Profiler/layout.html.twig' %}*/
/* */
/* {% block toolbar %}*/
/* <style>*/
/* .sf-minitoolbar {*/
/*     background-color: transparent;*/
/*     background-image: url(data:image/png;base64,) ;*/
/*     background-size: 32px 32px;*/
/*     background-repeat: no-repeat;*/
/* }*/
/* .sf-minitoolbar img {*/
/*     border: 1px dashed #0F0;*/
/*     opacity: 0;*/
/* }*/
/* </style>*/
/* */
/*     {% set icon %}*/
/*         <img width="32" height="32" alt="Bolt" style="width: 32px; height: 32px; margin-top: -2px;"*/
/*             src="data:image/png;base64,">*/
/* */
/*         <span></span><strong>Bolt {{ collector.version }}</strong></span>*/
/* */
/*         {% if collector.versionname is not empty %}*/
/*             <span class="sf-toolbar-status sf-toolbar-status-yellow">{{ collector.versionname }}</span>*/
/*         {% endif %}*/
/* */
/*     {% endset %}*/
/* */
/*     {% set text %}*/
/* */
/*         <div class="sf-toolbar-info-piece">*/
/* */
/*             <strong>Bolt - </strong> {{ collector.payoff }}<br>*/
/* */
/*             {{ collector.fullversion }} - {{ collector.aboutlink|raw }} - Visit <a href="http://bolt.cm" target="_blank">Bolt.cm</a></strong> <br>*/
/* */
/*             {% if collector.branding is not empty %}*/
/*                 {{ collector.branding|raw }}*/
/*             {% endif %}*/
/* */
/*         </div>*/
/* */
/*         {% if collector.editlink %}*/
/*         <div class="sf-toolbar-info-piece">*/
/*             <hr style="margin: 0;">*/
/*             Edit: <a href="{{ collector.editlink }}">{{ collector.edittitle|excerpt(40)|default("(no title)") }}</a>*/
/*         </div>*/
/*         {% endif %}*/
/* */
/* */
/*     {% endset %}*/
/*     {% include '@WebProfiler/Profiler/toolbar_item.html.twig' with { 'link': profiler_url } %}*/
/* {% endblock %}*/
/* */
/* */
